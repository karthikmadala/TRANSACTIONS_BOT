...
class Config:
    ...
    # Alternative testnet node: "https://data-seed-prebsc-1-s1.binance.org:8545"
    POLLING_INTERVAL: int = 60
    REQUEST_TIMEOUT: int = 10
    MAX_RETRIES: int = 3
    FALLBACK_PRICE: float = 0.0001  # Fallback price in USD
    FALLBACK_SUPPLY: float = 1000000  # Fallback initial supply
...
def is_new_holder(from_address: str, cache: Optional[Cache] = None) -> bool:
    if cache is None:
        logger.warning("Cache is None in is_new_holder, assuming new holder")
        return True
    logger.debug(f"Checking if {from_address} is a new holder. Known addresses: {len(cache.known_addresses)}")
    if from_address in cache.known_addresses:
        return False
...
async def send_to_telegram(
    ...
    holders_count = len(cache.known_addresses) if cache is not None else 0
    new_holder = is_new_holder(transaction.get("from", ""), cache)
    
    message = (
        f"<b>🚀 {name} Token ({symbol}) 🚀</b>\n\n"
...
async def calculate_24h_volume(transactions: List[Dict], decimals: int = Config.FALLBACK_DECIMALS) -> float:
    ...
    volume = sum(
        int(tx.get("value", 0)) / 10**decimals
        for tx in transactions
        if int(tx.get("timeStamp", 0)) >= one_day_ago and tx.get("value")
    )
        ...
        while True:
            try:
                ...
                if transactions:
                    ...
                    for tx in transactions[:5]:  # Log the latest 5 transactions for debugging
                    ...
                    if latest_tx["hash"] != cache.last_tx_hash:
                        ...
                        if latest_tx.get("from"):
                            is_new_holder(latest_tx["from"], cache)
