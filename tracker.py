...
class Cache:
    def __init__(self):
        ...
        self.holder_addresses = set()  # Renamed from known_addresses to be more specific
        ...
        try:
            if os.path.exists(self.cache_file):
                with open(self.cache_file, 'r') as f:
                    data = json.load(f)
                    self.holder_addresses = set(data.get('holder_addresses', []))
                    self.last_tx_hash = data.get('last_tx_hash')
                logger.info(f"Cache loaded successfully with {len(self.holder_addresses)} holder addresses")
    ...
    def save_cache(self):
        try:
            with open(self.cache_file, 'w') as f:
                json.dump({
                    'holder_addresses': list(self.holder_addresses),
                    'last_tx_hash': self.last_tx_hash
                }, f)
            logger.info(f"Cache saved successfully with {len(self.holder_addresses)} holder addresses")
        except Exception as e:
...
async def get_token_transactions(session: aiohttp.ClientSession) -> List[Dict]:
    url = f"https://api-testnet.bscscan.com/api?module=account&action=tokentx&contractaddress={Config.ICO_ADDRESS}&sort=desc&apikey={Config.BSCSCAN_API_KEY}"
...
async def get_token_holders(session: aiohttp.ClientSession, cache: Cache) -> Optional[int]:
    url = f"https://api-testnet.bscscan.com/api?module=token&action=tokenholderlist&contractaddress={Config.TOKEN_ADDRESS}&apikey={Config.BSCSCAN_API_KEY}"
    data = await fetch_with_retry(url, session)
    if data and data.get("status") == "1" and data.get("result"):
        holders = data["result"]
        holders_count = len(holders)
        # Update cache with unique holder addresses
        new_addresses = {holder["address"] for holder in holders}
        cache.holder_addresses.update(new_addresses)
        cache.save_cache()
        logger.info(f"Current holders: {holders_count}, Total unique holder addresses in cache: {len(cache.holder_addresses)}")
...
def is_new_holder(to_address: str, transactions: List[Dict], cache: Cache) -> bool:
    if to_address in cache.holder_addresses:
        return False
    for tx in transactions:
        cache.holder_addresses.add(tx["from"])
        cache.holder_addresses.add(tx["to"])
    cache.save_cache()
    return to_address not in cache.holder_addresses
...
async def send_to_telegram(
    ...
    # Enhanced message with unique holder count
    message = (
        ...
        f"👤 Unique Holders: {unique_holders_count:,}\n"
        ...
        while True:
            try:
                if time.time() - last_holder_update >= Config.HOLDER_UPDATE_INTERVAL:
                    holders_count = await get_token_holders(session, cache)
                ...
                if transactions:
                    ...
                    if latest_tx["hash"] != cache.last_tx_hash:
                        await send_to_telegram(
                            latest_tx,
                            holders_count,
                            len(cache.holder_addresses),  # Pass unique holder count
                            initial_supply,
                            price,
                            name,
                            symbol,
